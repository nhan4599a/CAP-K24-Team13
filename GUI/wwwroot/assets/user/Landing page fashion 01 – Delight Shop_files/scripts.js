		...
		if(cart) {
			...
			jQuery.each(cart.items,function(i,item){
				...
				if(item.image != null)
					...
					tr.find('.item-image').html("<img src='//theme.hstatic.net/1000306140/1000709257/14/no_image.jpg?v=2274'>");
...
function clone_item(product,i){
	...
	if ( product.image == null ) {
		item_product.find('img').attr('src','//theme.hstatic.net/1000306140/1000709257/14/no_image.jpg?v=2274').attr('alt', product.url);
	} else {
...
/*=============================*/
var nCount = 0;
$('.ldpage-technology01-flashsale').each(function(){
...
var HRT = {
	...
	initViews: function() {
		...
		switch (view) {
			...
			case 'page':
				break;
			case 'page.ldp-technology-01':
				that.LdpTechnology01.init();
				break;
			case 'cart':
				break;
			default:
...
HRT.LdpTechnology01 = { 
	...
	totalPag : [],
	init: function() {
		this.countdownTechnology01();
		this.copyCodeFlashSale();
		this.popoverCoupon();
		this.ajaxCollectionFlashSale();
	},
	countdownTechnology01: function(){
	...
	ajaxCollectionFlashSale: function() {	
		...
		$('.ldpage-technology01-flashsale').each(function(){
			...
			$(this).find('.collection-navtabs-title li a[data-toggle="tab"]').on('shown.bs.tab', function(event){
				...
				if(jQuery(window).width() < 768){
					...
					HRT.LdpTechnology01.scrollCenterFlashSale($parentScroll,".active", 500);
				}
				sectionCurrent.find(".tab-pane.active .icon-loading.tab-index").show();
				if(sectionCurrent.find('.tab-pane.active .collection-list-prd .product-loop-ldpage').length == 0){
					if(handle == '' ){
						jQuery.ajax({
							url: '/collections/all?view=ldp-technology-noproduct',
							success:function(data){
								...
								setTimeout(function(){								
									...
									sectionCurrent.find(".tab-pane.active").attr('data-get', 'true').find('.collection-list-prd').html(data);	
									setTimeout(function(){ jQuery(window).resize();	}, 400);
								}, 200);
							}
						});
						HRT.LdpTechnology01.totalPag[indexTab - 1] = null;
						HRT.LdpTechnology01.currentPag[indexTab - 1] = null;
					}
					else{
						currentHandle = handle;	
						if(HRT.LdpTechnology01.totalPag[indexTab - 1] == null){
							var cur_page = 1;
							HRT.LdpTechnology01.currentPag[indexTab - 1] = 1;
							sectionCurrent.find('.tablist-product-content .tab-pane.active .collection-loadmore').html(html_loadmore);
							jQuery.ajax({
								url:  currentHandle + '?view=ldp-technology-pagesize',	
								async: false,
								success:function(data){
									HRT.LdpTechnology01.totalPag[indexTab - 1] = parseInt(data);
									total_page = HRT.LdpTechnology01.totalPag[indexTab - 1];
								}
							});

							jQuery.ajax({
								url: currentHandle + '?view=ldp-technology-data&page=1',					
								success:function(data){
									sectionCurrent.find('.icon-loading.tab-index').hide();		
									setTimeout(function(){
										sectionCurrent.find('.tablist-product-content .tab-pane.active').attr('data-get', 'true').find('.collection-list-prd').html(data);				
										if( total_page > 1){
		...
		$(document).on('click', '.collection-loadmore .btn-loadmore', function(event){ 
			...
			var cur_page = HRT.LdpTechnology01.currentPag[indexTab];
			currentHandle = $('.collection-navtabs-title a[href="#'+idTab+'"]').attr('data-handle');
			if(HRT.LdpTechnology01.totalPag[indexTab] == null || HRT.LdpTechnology01.totalPag.length == 0 ){
				jQuery.ajax({
					url:  currentHandle + '?view=ldp-technology-pagesize',	
					async: false,
					success:function(data){
						HRT.LdpTechnology01.totalPag[indexTab] = parseInt(data);
						total_page = HRT.LdpTechnology01.totalPag[indexTab];
					}
				});
			}
			else{
			...
			cur_page++;	
			HRT.LdpTechnology01.currentPag[indexTab] = cur_page;
			if(!isloading && cur_page <= total_page){
				...
				isloading = true;			
				jQuery.ajax({
					url: currentHandle + '?view=ldp-technology-data&page=' + cur_page,
					success:function(data){
						setTimeout(function(){ 
							$('#'+idTab).find(".collection-list-prd").append(data);
							isloading = false;		
							if( total_page > 0){
								if(cur_page == total_page){
	...
	copyCodeFlashSale: function(){
		...
		$(document).on('click', '.popover-content__coupon .popover-code', function(e){ 
			e.preventDefault();	
			var btnPopover= $(this).attr('data-coupon');
			$(".coupon-item .cpi-button[data-coupon="+btnPopover+"]").click();
		});
	...
	popoverCoupon: function(){
		...
		$(popover).popover().on("shown.bs.popover", function(){	
			$('.popover-content__coupon .btn-popover-close,.modal-coupon--backdrop').click(function() {
